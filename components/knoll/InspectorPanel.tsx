...
import { getAgent, getChildNode } from '@/lib/knoll/agents'
...
export function InspectorPanel() {
  ...
  const childResult = !agent ? getChildNode(selectedNodeId) : undefined
  const child = childResult?.child
  const parentAgent = childResult?.agent
    ...
    <aside className="w-80 h-full bg-black/95 border-l border-gray-800 flex flex-col overflow-hidden flex-shrink-0">
      ...
      {agent && (
        <div className="flex-1 overflow-y-auto p-4 space-y-5">
          <div>
            <div className="text-[10px] font-mono uppercase tracking-[0.2em] mb-1" style={{ color: agent.primaryColor }}>
              {agent.class} · {agent.status}
            </div>
            <div className="text-xl font-bold text-white">{agent.name}</div>
            <div className="text-xs text-gray-500 italic mt-0.5">{agent.tagline}</div>
          ...
          <div>
            ...
            <div className="flex flex-wrap gap-1.5">
              {agent.capabilities.map(c => (
                <span key={c} className="text-[10px] font-mono px-2 py-0.5 rounded-full bg-gray-900 border border-gray-800 text-gray-300">{c}</span>
              ))}
            </div>
          </div>

          <div>
            <div className="text-[10px] font-mono text-gray-600 uppercase tracking-widest mb-2">Tools ({agent.tools.length})</div>
          ...
          <div>
            <div className="text-[10px] font-mono text-gray-600 uppercase tracking-widest mb-2">Child Nodes ({agent.children.length})</div>
            <div className="space-y-1">
              {agent.children.map(c => (
                <div key={c.id} className="flex items-center gap-2 px-3 py-1.5 rounded-lg bg-gray-900/40 border border-gray-800">
                  <div className="h-1.5 w-1.5 rounded-full flex-shrink-0" style={{ backgroundColor: agent.primaryColor }} />
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-5">
          <div>
            <div
              ...
              style={{ color: parentAgent?.primaryColor ?? '#6b7280' }}
            >
          ...
          <div>
            ...
            <span className="text-xs font-mono" style={{ color: parentAgent?.primaryColor ?? '#6b7280' }}>{child.memoryType}</span>
          </div>
          ...
          <div>
            ...
            <div className="flex flex-wrap gap-1.5">
              {child.capabilities.map(cap => (
                <span key={cap} className="text-[10px] font-mono px-2 py-0.5 rounded-full bg-gray-900 border border-gray-800 text-gray-300">{cap}</span>
              ))}
            </div>
          </div>

          <div>
            <div className="text-[10px] font-mono text-gray-600 uppercase tracking-widest mb-2">Tools ({child.tools.length})</div>
            <div className="space-y-1.5">
              {child.tools.map(t => (
                <div key={t.id} className="px-3 py-2 rounded-lg bg-gray-900/60 border border-gray-800">
                  <div className="text-xs font-semibold text-white">{t.name}</div>
          ...
          {parentAgent && (
            <div>
              ...
              <div className="flex items-center gap-2">
                <div className="h-2 w-2 rounded-full" style={{ backgroundColor: parentAgent.primaryColor }} />
